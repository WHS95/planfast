/**
 * 유저플로우 AI 생성의 공용 부품 — 스키마·프롬프트·정규화.
 * 일반 라우트와 스트리밍 라우트가 같은 프롬프트·같은 정규화를 쓰게 한 곳에 둔다.
 */
import { z } from "zod";
...
import { FLOW_NODE_TYPES, rid, type Flow, type FlowEdge, type FlowFrame, type FlowNode } from "@/lib/types";
...
export type RawFlowNodes = z.infer<typeof flowSchema>["nodes"];
export type RawFlowEdges = z.infer<typeof flowSchema>["edges"];
export type RawFlowFrames = NonNullable<z.infer<typeof flowSchema>["frames"]>;

export interface FlowGenPlan { mode: "new" | "revise"; base: Flow | undefined; request: string; prompt: string; nameFrom: (generated?: string) => string }

/** 요청을 검사하고 프롬프트를 조립한다. 문제가 있으면 { error, status } 를 돌려준다. */
export function planFlowGen(projectId: string, body: { mode?: "new" | "revise"; request?: string; flowId?: string; name?: string }): FlowGenPlan | { error: string; status: number } {
  const p = projects.get(projectId);
  if (!p) return { error: "project not found", status: 404 };
  const mode = body.mode ?? "new";
  const ready = flowReadiness(p);
  if (!ready.ready) return { error: !ready.hasPrd ? "PRD 개요의 '한 줄 정의'를 먼저 작성하세요." : "기능명세서에 기능을 1개 이상 추가하세요.", status: 409 };
  const base = mode === "revise" && body.flowId ? flows.get(body.flowId) : undefined;
  if (mode === "revise" && (!base || base.projectId !== projectId)) return { error: "flowId required for revise", status: 400 };
  const { text } = projectContext(projectId, { includePages: true, includeFlows: false });
  ...
  const prompt = [
    ...
    guide,
    "출력 순서: name → frames → nodes → edges 순으로 쓰세요(화면에 순서대로 그리기 위해). 각 노드 객체는 id, type, label 을 먼저 쓰세요.",
  ].filter(Boolean).join("\n\n");
  const nameFrom = (generated?: string) => mode === "revise" ? `${(base!.name).replace(/\s*\(수정본( \d+)?\)$/, "")} (수정본)` : (body.name?.trim() || generated || "새 유저플로우");
  return { mode, base, request, prompt, nameFrom };
}

/** Ensure exactly one start, unique ids, valid edges, branch labels, frames, and initial positions. */
export function normalizeFlow(
  rawNodes: z.infer<typeof flowSchema>["nodes"],
  rawEdges: z.infer<typeof flowSchema>["edges"],
  rawFrames: NonNullable<z.infer<typeof flowSchema>["frames"]>,
