...
import { TYPE_CLASS, descendantIds, flattenVisible, matchesQuery } from "./utils";

type DropPos = "before" | "after" | "into";

export function DirectoryView() {
  const { store, selectedId, select, collapsed, toggleCollapse, query, currentMatchId, addChild } = useFeatures();
  ...
  return (
    <div className="flex-1 flex min-h-0">
      <div ref={listRef} className="w-[340px] shrink-0 border-r overflow-y-auto py-2">
        ...
        {rows.map(({ item, depth }) => {
          ...
          return (
            <div key={item.id} data-item-id={item.id} draggable
              ...
              className={clsx("group flex items-center gap-1 pr-2 py-1 text-sm cursor-pointer select-none relative",
              ...
              {isOver && over.pos === "before" && <div className="absolute left-2 right-2 top-0 h-0.5 bg-accent" />}
              {isOver && over.pos === "after" && <div className="absolute left-2 right-2 bottom-0 h-0.5 bg-accent" />}
              <GripVertical size={12} className="text-muted opacity-0 group-hover:opacity-100 shrink-0" />
              <button className={clsx("w-4 h-4 flex items-center justify-center shrink-0 text-muted", kids === 0 && "invisible")} onClick={(e) => { e.stopPropagation(); toggleCollapse(item.id); }}>
              ...
              <span className="hidden group-hover:inline"><StatusBadge status={item.status} /></span>
            </div>
          );
        })}
        <button className="btn btn-ghost btn-sm text-muted mx-2 mt-2" onClick={() => void addChild(null)}><Plus size={12} /> 요구사항 추가</button>
      </div>
      <div className="flex-1 min-w-0 flex flex-col">
        {selected ? (
          <ItemDetail item={selected} onPrev={idx > 0 ? () => select(rows[idx - 1].item.id) : undefined} onNext={idx >= 0 && idx < rows.length - 1 ? () => select(rows[idx + 1].item.id) : undefined} />
        ) : (
          <div className="flex-1 flex items-center justify-center text-sm text-muted">왼쪽 목록에서 항목을 선택하세요. 드래그하여 순서를 바꾸거나 다른 상위 항목으로 옮길 수 있습니다.</div>
        )}
      </div>
    </div>
  );
