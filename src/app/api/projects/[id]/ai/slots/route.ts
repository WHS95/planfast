...
import { SPEC_SLOTS, SPEC_SLOT_LABEL, type FeatureData, type SpecData, type SpecSlot } from "@/lib/types";
...
/** POST { itemId: string, hint?: string } → { slots: Record<SpecSlot,string> } */
export const POST = handler(async (req, { params }: Params<"id">) => {
  ...
  if (!spec || spec.projectId !== id) return notFound("item not found");
  if (spec.type !== "spec") return bad("상세기능 항목만 슬롯을 작성할 수 있습니다");
  ...
  const siblings = feature ? items.list(id).filter((x) => x.parentId === feature.id && x.id !== spec.id).map((x) => `- ${x.title}`).join("\n") : "";
